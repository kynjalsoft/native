...
5. Live Stalwart-to-relay-to-Expo subscription, provider receipt, and foreground/background/terminated device presentation for iOS and Android. Verify revocation, logout, account switching, relay outage, preview and exact-tap behavior. A provider ticket is not device presentation. Follow the full matrix and current backend evidence in the [notification lifecycle audit](notification-lifecycle-audit-2026-09-26.md).
...
The production relay origin in `eas.json` is build-time configuration, not release activation. Do not create or distribute a production-profile signed release build, publish a production update, or submit to a store until signed iOS and Android physical-device preview and exact-tap qualification is recorded. Internal signed qualification builds are for those tests only.
